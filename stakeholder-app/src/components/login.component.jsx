...
const LoginComponent = () => {
  ...
  const handleValidSubmit = async (data) => {
    ...
    try {
      ...
      if (result.data) {
        const {authToken, registrationToken}  = result.data
        if(authToken ){
          navigate('/profile')
        }
        else{
          navigate(`/register/${registrationToken}`)
        }
      }
    } catch (error) {
      ...
      <div className="col-6 offset-3">
        <form onSubmit={handleSubmit(handleValidSubmit)}>
          <div className="mb-3">
            ...
            <input type="email" className="form-control" id="inputEmail" {...register('userName')} />
